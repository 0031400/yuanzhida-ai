import { getMyCollectQuestionPage } from '../../api/question'
import { authStore } from '../../store/auth.store'
import type { QuestionItem } from '../../types/question'
import { formatFromNow } from '../../utils/day'
import { pickErrorMessage } from '../../utils/error'

interface CollectQuestionCard extends QuestionItem {
...
Page({
  data: {
    ...
    questions: [] as CollectQuestionCard[],
  },
  onShow() {
  ...
  mapToCard(question: QuestionItem): CollectQuestionCard {
    return {
      ...question,
  ...
  async loadQuestions(reset: boolean): Promise<void> {
    ...
    try {
      const page = await getMyCollectQuestionPage(current, this.data.size)
      ...
      wx.showToast({
        title: pickErrorMessage(error, '我的收藏加载失败'),
        icon: 'none',
      })
    } finally {
      this.setData({
