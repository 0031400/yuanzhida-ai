...
Page({
  ...
  mapToCard(question: QuestionItem): MyQuestionCard {
    return {
      ...question,
  ...
  async loadQuestions(reset: boolean): Promise<void> {
    ...
    try {
      const page = await getMyQuestionPage(current, this.data.size)
      ...
      wx.showToast({
        title: pickErrorMessage(error, '我的提问加载失败'),
        icon: 'none',
      })
    } finally {
      this.setData({
